...
`1-domain-definition.md`(v1.5)의 핵심 액터·핵심 유스케이스를 기반으로 작성했다. Mermaid는 UML 유스케이스 다이어그램을 네이티브로 지원하지 않아 `flowchart`로 표현했다(액터: 사각형, 유스케이스: 타원). 이벤트/참여신청의 상태 전이(등록→진행중→종료 등)는 이 문서의 범위가 아니며, 별도 상태 다이어그램에서 다룬다.
