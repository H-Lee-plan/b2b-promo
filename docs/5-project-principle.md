...
- 버전: v1.1 (2026-08-13) — 참여신청 중복 판정을 UNIQUE 위반 예외 catch 방식에서 `INSERT ... ON CONFLICT`로 교체(예외 catch는 PostgreSQL 트랜잭션을 abort시켜 같은 트랜잭션 내 룰렛 추첨이 실패하는 문제가 있었음), 쿼리 함수의 트랜잭션 재사용 패턴(pool/client 공용 인터페이스, release 누락 방지) 명시
